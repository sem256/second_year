...
namespace Lab_2
{
    public partial class Form1 : Form
    {
        ...
        {
            Regex R = new Regex("(0[1-9]|1[0-9]|2[0-9]|3[01])-(0[1-9]|1[012])-[0-9]{4}");
        ...
        private void run_query_Click(object sender, EventArgs e)
        {
            ...
            if (radioButtonAdd.Checked)
            {
                ...
                else if (!checkedData(textBoxBirth.Text)) MessageBox.Show("YYYY-MM-DD");
                //else if (comboBoxDisease.Enabled) MessageBox.Show("Виберіть Хворобу паціента");
                else
                {
                    //flag = true;
                    command = "insert Hospital_Patients values ('" + textBoxName.Text + "'" + "," + "'" + textBoxMiddleName.Text
            ...
            {
                MessageBox.Show("Запит виконаний не вірно повторіть ще раз");
            }
        }
        ...
        public void diseaseList()
        {
            ...
            {
                connection.Open();
                SqlDataAdapter add = new SqlDataAdapter("select * from Infectious_Diseases", connectionString);
                add.Fill(data);
            }
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            if (!t) 
            {
                ...
                {
                    connection.Open();
                    SqlDataAdapter add = new SqlDataAdapter("insert Infectious_Diseases values ('Туберкульоз','Збудник - палочка Коха')", connectionString);
                    add.Fill(data);
                }
            }
            if (!d)
            {
                ...
                {
                    connection.Open();
                    SqlDataAdapter add = new SqlDataAdapter("insert Infectious_Diseases values ('Дифтерія','Небезпечна дитяча хвороба')", connectionString);
                    add.Fill(data);
                }
            }
        }
